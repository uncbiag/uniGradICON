...
for case in tqdm(test_cases):
    ...
    os.makedirs(f"{case_result_save_dir}/ES_ED/ants/", exist_ok=True)

    # Run ants registration
    _, warped_seg, reg_res = run_ants(fixed, moving, args.transform_type, f"{case_result_save_dir}/ES_ED/ants/",
                                             fixed_mask=fixed_seg, moving_mask=moving_seg, return_disp_tensor=False)
    ...
    if bidirection:
        ...
        os.makedirs(f"{case_result_save_dir}/ED_ES/ants/", exist_ok=True)
        
        # Run ants registration
        _, warped_seg, reg_res = run_ants(fixed, moving, args.transform_type, f"{case_result_save_dir}/ED_ES/ants/",
                                                fixed_mask=fixed_seg, moving_mask=moving_seg, return_disp_tensor=False)
