...
if args.exp == "":
    run_name = f"ants/{args.transform_type}/OAI"
else:
    run_name = f"{args.exp}/{args.transform_type}/OAI"
    if "[" in run_name:
        run_name = run_name.replace("[", "_").replace("]", "_")
    
footsteps.initialize(output_root="evaluation_results/", run_name=run_name)
