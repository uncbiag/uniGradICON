...
if args.exp == "":
    ...
    footsteps.initialize(output_root="evaluation_results/", run_name=f"{args.exp}/L2R_OASIS")
...
for (fixed_path, moving_path) in tqdm(test_cases):
    ...
    fixed = np.asarray(nib.load(os.path.join(args.data_folder, fixed_path)).get_fdata()).copy()
    moving = np.asarray(nib.load(os.path.join(args.data_folder, moving_path)).get_fdata()).copy()
    ...
    np.savez_compressed(f"{footsteps.output_dir}/submission/submission/task_03/disp_{case_id}.npz", disp)
