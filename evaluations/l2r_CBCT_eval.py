...
from unigradicon import make_network, make_sim
...
parser.add_argument("--io_sim", type=str, default="lncc", help="Similarity measure used in IO.")

origin_shape = [1, 1, 300, 280, 390]
...
if args.exp == "":
    ...
    footsteps.initialize(output_root="evaluation_results/", run_name=f"{args.exp}/L2R_CBCT")

logger = utils.Logger(f"{footsteps.output_dir}/output.txt")

os.makedirs(f"{footsteps.output_dir}/submission/cbct", exist_ok=True)

net = make_network(input_shape, include_last_step=True, loss_fn=make_sim(args.io_sim))
...
for (fixed_path, moving_path) in tqdm(test_cases):
    ...
    nib.save(disp_itk_format, f"{footsteps.output_dir}/submission/cbct/disp_{case_id}.nii.gz")
